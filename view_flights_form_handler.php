...
<html>
	...
	<body style='background-color: #0077c82e;'>
		...
		<div>
			<ul>
				<li style="margin-left: 45%;"><a href="landing_page.php"><i class="fa fa-home" aria-hidden="true"></i> Home</a></li>
			</ul>
		</div>
		...
		<?php
			if(isset($_POST['Search']))
			{
				...
				if(empty($data_missing))
				{
					...
					if($class=="economy")
					{
						$query="SELECT flight_no,from_city,to_city,departure_date,departure_time,arrival_date,arrival_time,price_economy FROM flight_det where from_city=? and to_city=? and departure_date=? and seats_economy>=? ORDER BY  departure_time";
					...
					{
						$query="SELECT flight_no,from_city,to_city,departure_date,departure_time,arrival_date,arrival_time,price_business FROM flight_det where from_city=? and to_city=? and departure_date=? and seats_business>=? ORDER BY  departure_time";
						$stmt=mysqli_prepare($dbc,$query);
						mysqli_stmt_bind_param($stmt,"sssi",$origin,$destination,$dep_date,$no_of_pass);
						mysqli_stmt_execute($stmt);
